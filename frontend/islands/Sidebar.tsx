...
export default function Sidebar({ mobileOpen, setMobileOpen }: { mobileOpen: boolean; setMobileOpen: (v: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    const checkMobile = () => {
      ...
      if (mobile) {
        ...
        const desktopOpen = savedState !== "true";
  ...
  if (!mounted) {
    return <aside class="fixed top-0 left-0 h-full w-64 bg-white dark:bg-gray-800 border-r dark:border-gray-700 z-40" />;
  }

  const displayOpen = isMobile ? mobileOpen : isOpen;
    ...
    <>
      ...
      <aside
        class={`fixed top-0 left-0 h-full bg-white dark:bg-gray-800 border-r dark:border-gray-700 transition-all duration-300 ease-in-out z-40 flex flex-col ${
          displayOpen ? "w-64" : "w-20"
        } ${isMobile && !mobileOpen ? "-translate-x-full" : "translate-x-0"}`}
      >
        <div class="h-16 flex items-center justify-between px-4 border-b dark:border-gray-700 flex-shrink-0">
          <div class={`flex items-center gap-2 overflow-hidden transition-all duration-300 ${displayOpen ? "w-auto opacity-100" : "w-0 opacity-0"}`}>
          ...
          <button
            ...
            aria-label={displayOpen ? "Tutup sidebar" : "Buka sidebar"}
          >
            <svg 
              class={`w-5 h-5 transition-transform duration-300 ${displayOpen ? "rotate-0" : "rotate-180"}`}
              fill="none" 
              stroke="currentColor" 
              viewBox="0 0 24 24"
            >
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span class={`font-medium whitespace-nowrap transition-all duration-300 ${displayOpen ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4 absolute"}`}>
                  {item.label}
                </span>
                
                {!displayOpen && !isMobile && (
